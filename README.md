...
- **Data flow**:
    - Front-end consome hooks (ex.: `useHomeData()`, `useAllPosts()`, `usePostBySlug()`).
    - Hooks chamam o proxy serverless `/api/cms/[...path]` (no Next API Routes / Edge Functions) servindo como integração com o ambiente Prismic a fim de tratar dados e aplica cache/headers (stale-while-revalidate).
- **Cache & ISR**:
